...
import { addThought } from './index.js';
...
describe('addThought', () => {
	...
	it('should add a basic thought to history', async () => {
		const input = {
		...
		const result = await addThought(input, context);
	...
	it('should store thought in history', async () => {
		...
		await addThought(input, context);
	...
	it('should return error response on exception', async () => {
		const result = await addThought(null as any, null as any);

		expect(result.isError).toBe(true);
		expect(result.content[0].text).toContain('error');
	});

	it('should track multiple thoughts sequentially', async () => {
		for (let i = 1; i <= 3; i++) {
			await addThought(
				{
					available_mcp_tools: ['tool1'],
		...
		expect(context.thought_history).toHaveLength(3);
		expect(context.thought_history[0].thought).toBe('Thought 1');
		expect(context.thought_history[1].thought).toBe('Thought 2');
		expect(context.thought_history[2].thought).toBe('Thought 3');
	});

	it('should add multiple thoughts to the same branch', async () => {
		const inputs = [
			{
				available_mcp_tools: ['tool1'],
