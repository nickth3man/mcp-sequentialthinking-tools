...
export const ADD_THOUGHT_INPUT_SCHEMA = z.object({
	available_mcp_tools: z.array(z.string().max(256)).max(1000).describe('Array of MCP tool names available for use'),
	thought: z.string().max(100000).describe('The thought content to add'),
	...
	current_step: z.object({
		...
		recommended_tools: z.array(z.object({
			...
			priority: z.number(),
			suggested_inputs: z.record(z.string().max(256), z.unknown()).optional(),
			alternatives: z.array(z.string().max(256)).max(100).optional(),
		})).max(100),
		expected_outcome: z.string().max(10000),
		next_step_conditions: z.array(z.string().max(1000)).max(100).optional(),
	}).optional().describe('Current step recommendation'),
	previous_steps: z.array(z.object({
		step_description: z.string().max(10000),
		recommended_tools: z.array(z.object({
			tool_name: z.string().max(256),
			confidence: z.number().min(0).max(1),
			rationale: z.string().max(10000),
		...
		expected_outcome: z.string().max(10000),
		next_step_conditions: z.array(z.string().max(1000)).max(100).optional(),
	})).max(1000).optional().describe('Steps already recommended'),
